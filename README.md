...
除“今日桃签”和别名“桃签”外，是否需要命令前缀取决于机器人项目的
`COMMAND_START` 配置。下表用 `/` 演示普通命令；“今日桃签”直接发送文本即可。
...
| `今日桃签`（或 `桃签`） | 所有人 | 群聊/私聊 | 无需 `/`；同账号当天固定、次日重新抽取 |
